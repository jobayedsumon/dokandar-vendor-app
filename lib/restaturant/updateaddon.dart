...
class UpdateAddOnRest extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: UpdateAddOnRest1(addonid, addon_price, addon_name),
    );
  }
}

class UpdateAddOnRest1 extends StatefulWidget {
  final dynamic addonid;
  final dynamic addon_price;
  final dynamic addon_name;

  UpdateAddOnRest1(this.addonid, this.addon_price, this.addon_name);

  @override
  State<StatefulWidget> createState() {
    return UpdateRestAddState();
  }
}

class UpdateRestAddState extends State<UpdateAddOnRest1> {
  dynamic currency;
  ...
  void newHitService(ProgressDialog pr, BuildContext context) async {
    if (productPriceC.text.isNotEmpty && productNameC.text.isNotEmpty) {
      pr.show();
      var storeEditUrl = resturant_addaddons_update;
