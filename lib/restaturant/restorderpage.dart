...
class _OrderPageRestState extends State<OrderPageRest>
  ...
  void initState() {
    super.initState();
    getSharedPref();
    print('CALING STATUS SERVICE FUNCTION');
    hitStatusServiced();
    print('END STATUS SERVICE FUNCTION');
    getTodayOrders();
    setTimerTask();
  }
  ...
  hitStatusServiced() async {
    print('INSIDE STATUS SERVICE');
    setState(() {
      isRun = true;
    });
    ...
    await client
        .post(statusUrl, body: {'vendor_id': '${vendorId}'}).then((value) {
      setState(() {
        isRun = false;
      });
      if (value.statusCode == 200 && value.body != null) {
        ...
        if (jsonData['status'] == 1) {
          ...
          print('----ONLINE_STATUS  --${sats.toString()}');
          ...
          print('JSON DATA STATUS: ${jsonData['status'] == "1"}');
        }
        Toast.show(jsonData['message'], context,
            duration: Toast.LENGTH_SHORT, gravity: Toast.CENTER);
      } else {
        print("ERROR FETCHING STATUS ON IF ELSE\n");
      }
    }).catchError((e) {
